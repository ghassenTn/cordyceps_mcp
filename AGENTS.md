...
- **`src/structure/`** — the tools' logic, pure over the graph client:
  - `index_view.py` — `IndexView` = one `get_all_metadata()` snapshot per request + lookup tables; the *only* place that parses node IDs. `normalize_path()` handles `.`/`./`/absolute-in-workspace paths. Also owns the inline-callback rules (`is_inline_callback`, `named_ancestor`, `visible_members`, `display_name`, `named_symbols_in_file`).
  - `resolver.py` — `resolve_symbol()` → `Resolution(status: resolved|ambiguous|not_found)`. Tiers: exact id → `<file>:<qual>` shorthand → bare/dotted name → file/folder path → case-insensitive. Qualified matching accepts both the raw qualified name and the display name. **Never picks one of several matches**; ambiguity is returned as candidates.
  - `records.py` — allow-listed projections (`brief_record`, `symbol_record`, `compact_entry`, `group_by_file`, `import_statements`, `export_entries`). No `body`, no `extra_json`, no raw `calls`. `is_noise_call()` filters builtins/platform globals/`callback_L*` handles from `unresolved_calls`.
  - `service.py` — `CodeStructure.map/lookup/impact`. Bounds: `MAX_MAP_ENTRIES=300`, `MAX_RELATED=50`, `MAX_IMPACT_NODES=300`, `MAX_IMPACT_DEPTH=25`. `impact` is a Python BFS over `_flat_neighbours()` (callers/callees with inline callbacks made transparent) recording hop depth; it reports `truncated` and `more_beyond_depth` explicitly. Direct neighbours are the `depth=1` entries of `affected` (no separate list).
...
- **`src/database/parser/`** — `UniversalCodeParser` (tree-sitter). `languages/*.yaml` register `.py`, `.js/.jsx`, `.ts/.tsx`. Non-code extensions are body-only `File` nodes. JS/TS `arrow_wrapper_functions` (forwardRef, memo, observer, useCallback) names the declarator's binding and lets it own the wrapped literal's body calls (`_declared_function`/`_wrapped_function` in `ast_parser.py`).
...
- **Node IDs:** `rel_file_path:Qualified.Name` (methods/nested defs), `rel_file_path:name` (declarations), `rel_file_path:<url>` (routes, may end with `/`), `rel_file_path:mw:<name>` (middleware), files = `rel_file_path`, folders = `relative/folder`. Inline callbacks nest as `rel_file_path:Outer.callback_L<line>_C<col>` (JS/TS only).
- **Inline callbacks are transparent in every tool** (graph keeps them for scope precision; output never surfaces them): `code_map` hoists their named children and counts them in `meta.inline_callbacks`; `lookup` attributes their call edges to the enclosing definition; `impact` hops through them (depth counts named definitions only); `lookup`/`impact` print the display name (`App.checkBackend`) while `id` keeps the full path. `const X = useCallback/memo/forwardRef/observer(...)` *defines* `X` (named node, owns the body calls); a bare `items.map(x => ...)` callback never is.
